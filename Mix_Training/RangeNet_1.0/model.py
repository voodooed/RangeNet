...
class Darknet(torch.nn.Module):
    def __init__(self, layers_number=21, in_channels=1, out_channels=1, momentum=0.9, slope=0.1, dropout_p=0.01):
...
def test():
    x = torch.randn((12, 1, 64, 1024))
    model = Darknet(layers_number=21, in_channels=1, out_channels=20, momentum=0.9, slope=0.1, dropout_p=0.01)
    preds = model(x)
    print(preds.shape)
    print(x.shape)
